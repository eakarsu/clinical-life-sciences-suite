const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:3610';
...
const adminCookie = await login('admin@clinical-life.local', 'admin123');
const managerCookie = await login('manager@clinical-life.local', 'manager123');
const analystCookie = await login('analyst@clinical-life.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/studies', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const studies = await expectJson('/api/entities/studies', managerCookie);
const rowId = studies.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/studies/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/studies/approve`, {
  method: 'POST',
  headers: {
